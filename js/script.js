window.addEventListener('DOMContentLoaded', function() { // функция которая ждет пока вся страница загрузиться прежде чем выполнять код скрипта
    ...
    const loadContent = async (url, callback) => {
        await fetch(url) // ждет пока fetch создаст карточки товара и только потом продолжает запуска остальной js
        .then(response => response.json()) // превращение из json в обычный js
        .then(json => createElement(json.goods));

        callback();
    }

    function createElement(arr) { // массив для получения данных из БД

        const goodsWrapper = document.querySelector ('.goods__wrapper'); // получаем обертку для карточек товаров

        arr.forEach(function(item) {
            let card = document.createElement('div'); // создаем пустой блок для карточки товара
            card.classList.add('goods__item'); // добавляем карточке класс
            card.innerHTML = `
            ...
            `; // вставляем html код верстка
            goodsWrapper.appendChild(card); // добавление карточки
        });
    }
    ...
    loadContent('js/db.json', () => {
        ...
        function openCart() {
            cart.style.display = 'block';                           // показывет корзины через свойство цсс для корзину
